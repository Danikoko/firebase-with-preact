...
export function App() {
    ...
    const [idToUpdateIsSetInitially, setIdToUpdateIsSetInitially] =
        useState(false);
    const [idToDelete, setIdToDelete] = useState('');
    const [idToDeleteIsSetInitially, setIdToDeleteIsSetInitially] =
        useState(false);
    ...
    const handleSetCurrentBook = (id: string, booksParam: any[] = books) => {
        const [value] = booksParam.filter(
            (bookItem: any) => bookItem.id === id
        );
        setCurrentBook(
            value || {
                id: '',
    ...
    const handleAddBook = (e: any) => {
        ...
        addBook(payload).then(({ success, errorInfo }: any) => {
            if (success) {
                setTitle('');
                setAuthor('');
            } else {
    ...
    const handleUpdateBook = (e: any) => {
        ...
        updateBook(payload).then(({ success, errorInfo }: any) => {
            if (success) {
                // setTitleForUpdate('');
                // setAuthorForUpdate('');
            } else {
    ...
    const handleDeleteBook = (e: any) => {
        e.preventDefault();
        deleteBook(idToDelete).then(({ success, errorInfo }: any) => {
            if (success) {
                if (books.length >= 1) {
        ...
        <>
            ...
            <form method='POST' onSubmit={(e: any) => handleUpdateBook(e)}>
                ...
                <div className='form-group'>
                    <label for='book-title'>Book Title</label>
                    <input
                        onInput={(e: any) => setTitleForUpdate(e.target.value)}
                        value={titleForUpdate}
                        type='text'
                        id='book-title-update'
                        name='book-title'
                        required
                    />
                </div>
                ...
                <div className='form-group'>
                    <label for='author-name'>Author Name</label>
                    <input
                        onInput={(e: any) => setAuthorForUpdate(e.target.value)}
                        value={authorForUpdate}
                        type='text'
                        id='author-name-update'
                        name='author-name'
                        required
                    />
                </div>
            ...
            <form method='POST' onSubmit={(e: any) => handleDeleteBook(e)}>
                <div className='form-group'>
                    ...
                    <select
                        onChange={(e: any) => setIdToDelete(e.target.value)}
                    ...
                    <form method='POST' onSubmit={(e: any) => signIn(e)}>
                        <div className='form-group'>
                            ...
                            <input
                                ...
                                value={loginEmail}
                                type='text'
                                id='user-email-to-login'
                                name='user-email-to-login'
                                required
                            />
                        </div>
                        ...
                        <div className='form-group'>
                            ...
                            <input
                                ...
                                value={loginPassword}
                                type='password'
                                id='user-password-to-login'
                                name='user-password-to-login'
                                required
                            />
                        </div>
